        ...
        let mutCode = solm.edit(data.toString(), function(node) {
            if(node.type === 'FunctionDeclaration'
                ...
                for(var i = 0; i < node.modifiers.length; i++) {
                    if(node.modifiers[i].name == 'public' || node.modifiers[i].name == 'private'
                        || node.modifiers[i].name == 'internal'|| node.modifiers[i].name == 'external'
                    ){visPos = i;}
                }
                ...
                fs.writeFileSync("./sol_output/" + filename + "/"
                    ...
                    node.parent.getSourceCode(), tmpNodeSC1), 'ascii');
                fileNum++;

                fs.writeFileSync("./sol_output/" + filename + "/"
                    + path.basename(file).slice(0, -4) + "FunctionVisMut"
                    + fileNum.toString() + ".sol", data.toString().replace(
                    node.parent.getSourceCode(), tmpNodeSC2), 'ascii');
                fileNum++;

                fs.writeFileSync("./sol_output/" + filename + "/"
                    + path.basename(file).slice(0, -4) + "FunctionVisMut"
                    + fileNum.toString() + ".sol", data.toString().replace(
                    node.parent.getSourceCode(), tmpNodeSC3), 'ascii');
                fileNum++;
            }
        });
    }
